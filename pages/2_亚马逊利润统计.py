...
class UI:
    ...
    def _display_instructions_tab(self):
        ...
        st.header("💡 使用说明与注意事项")
        st.markdown("""
        欢迎使用亚马逊成本利润计算器！本工具旨在帮助亚马逊卖家精确估算一票货物的总成本、预期利润及各项费用明细。
        """)
        ...
        **第一步：选择数据源**
        ...
        - **⚡ 快速开始**：点击此选项将加载预设的默认配置，适合新用户快速体验系统功能
        - **📁 导入配置文件**：如果您有之前导出的JSON配置文件，可以在此上传恢复您的设置

        **注意**：两种数据源方式二选一，确保数据纯净性。

        **第二步：填写配置参数 (在"⚙️ 费用配置"选项卡中)**

        选择数据源后，按照从左到右，从上到下的顺序依次填写所有相关参数：
        ...
        **第三步：查看分析结果**
        ...
        st.markdown("""
        **导出配置**
        - 当您完成一次完整的参数配置后，可以在 **"⚙️ 费用配置"** 选项卡中点击 **"💾 导出当前配置"** 按钮。
        - 这会将所有配置保存为一个 `.json` 文件，适合用于备份、存档或与团队成员分享。

        **重新选择数据源**
        - 如果您想切换数据源（比如从默认配置切换到文件导入，或者反之），可以点击 **"🔄 重新选择"** 按钮。
        - 这将清空当前所有数据，回到数据源选择界面。

        **数据持久化**
        - 您的所有输入都保存在当前浏览器会话中。刷新页面不会丢失数据，但关闭浏览器标签页后数据会丢失。
        - 建议在完成重要配置后及时使用导出功能保存到本地。
        """)

        st.subheader("📝 重要注意事项")
        st.markdown("""
        **数据源选择**
        - 首次使用时必须选择数据源，否则无法进行任何计算
        - 默认配置和文件导入是互斥的，确保数据一致性
        - 切换数据源会清空当前所有设置，请提前导出重要配置

        **单位一致性**
        - 请特别注意每个输入框后面的单位提示（例如 `元` vs `$`，`KG` vs `cm`）
        - 错误的单位将导致计算结果严重偏差
        ...
        **实时计算**
        - 页面上的所有计算都是实时触发的。每当您修改任何一个数字，整个计算链条都会自动重新运行
        - 如果发现计算结果异常，请检查相关参数的输入是否正确
        """)

        st.subheader("🆘 常见问题")
        st.markdown("""
        **Q: 为什么页面显示"请选择数据源"，没有计算界面？**
        A: 这是正常现象。首次使用时需要先选择数据源，点击"快速开始"或导入配置文件即可。

        **Q: 导入配置文件后数据没有变化？**
        A: 请检查文件格式是否正确，必须是之前从此系统导出的JSON格式文件。

        **Q: 如何重置所有设置？**
        A: 点击"重新选择数据源"按钮，或者直接刷新浏览器页面。

        **Q: 可以同时使用默认配置和自定义文件吗？**
        A: 不可以。系统设计为二选一模式，确保数据源的纯净性。

        **Q: 为什么SKU价格显示不正确？**
        A: 请确认您选择的数据源是否正确，或者检查导入的配置文件内容是否完整。
        """)

        st.success("""
        **现在开始使用：**
        1. 如果您是首次使用，建议选择"快速开始"体验功能
        2. 如果您有历史数据，请导入之前的配置文件
        3. 在"费用配置"选项卡中完善您的参数设置
        4. 查看分析结果并导出重要配置
        """)
