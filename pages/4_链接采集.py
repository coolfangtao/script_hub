...
class Parser1688(BasePlatformParser):
    ...
    def parse_beauty_ranking_list(self, html_content):
        """
        (已更新) 榜单文件 Tab -
        首选策略: (极快) 纯文本搜索 `offerId@...` 和 `object_id@...` (基于插件更新)
        备用策略: (较慢) 仅在首选失败时，才启动BeautifulSoup查找其他模式
        """
        found_ids = set()

        # 策略 1: (首选 - 速度极快) 直接在原始 HTML 文本中查找
        try:
            # 查找 'offerId@...'
        ...
        if not found_ids:
            try:
                ...
                # 策略 2b: (兜底) 查找 <a> 标签 (用于标准 offer/xxx.html 链接)
    ...
    def _build_1688_tab(self):
        ...
        with tab_rank_list:
            ...
            **使用方法:**
            ...
            uploaded_file_rank = st.file_uploader(  # (已修改)
                "上传由浏览器插件下载的 (ranking) HTML文件",
                type=["html", "htm"],
                key="1688_ranking_file"  # (已修改)
            )

            if st.button("开始解析 (榜单文件)", type="primary", key="btn_1688_rank"):  # (已修改)
                if uploaded_file_rank is not None and self.parser_1688:  # (已修改)
        ...
        with tab_category_list:
            ...
            **使用方法:**
            ...
            uploaded_file_cat = st.file_uploader(
                "上传由浏览器插件下载的 (category) HTML文件",  # (已修改说明文字)
                type=["html", "htm"],
